...
;(async () => {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (pathname.endsWith('/booking-details.html') && new URL(req.url, 'http://local').searchParams.has('authored-card')) {
        const authored = await fs.readFile(path.join(root, 'v3/fixtures/card-setup-owner.html'), 'utf8')
        const dialog = authored.slice(authored.indexOf('<dialog'), authored.indexOf('</dialog>') + '</dialog>'.length)
        body = Buffer.from(body.toString().replace(/<dialog popup-stripe-card[\s\S]*?<\/dialog>/, dialog))
      }
  ...
  try {
    ...
    await navigate('authored-card=1')
    await openDetails('paid')
    await fill(context, 'Authored hierarchy draft')
    await evaluate('fixture.failList = 1')
    await openPayment()
    const visible = selector => evaluate(`(() => { const node = document.querySelector(${JSON.stringify(selector)}); return !!node && node.getClientRects().length > 0 && getComputedStyle(node).visibility !== 'hidden' })()`)
    for (const selector of ['[data-booking-payment-picker]', '[data-booking-payment-disclosure]', '[aria-label="Add payment method"]', '[aria-label="Retry loading cards"]', '[data-booking-payment-back]']) {
      assert.equal(await visible(selector), true, selector + ' remains visible in the authored card wrapper')
    }
    assert.equal(await visible('[save-card-btn]'), false)
    await click('[aria-label="Retry loading cards"]')
    await waitFor(`document.querySelectorAll('[customer-cards-list] [role="radio"]').length === 2`)
    assert.equal(await visible('[customer-cards-list]'), true)
    await addCard()
    for (const selector of ['[card-element]', '[save-card-btn]', '[data-payment-card-back]', '[data-booking-payment-disclosure]']) assert.equal(await visible(selector), true)
    assert.equal(await evaluate(`getComputedStyle(document.querySelector('[pm-use-this]').parentNode).display`), 'none', 'only the inactive authored footer collapses')
    await click('[data-payment-card-back] button')
    await waitFor(`document.querySelectorAll('[customer-cards-list] [role="radio"]').length === 2`)
    assert.equal(await visible('[data-booking-payment-back]'), true)
    await chooseCard('pm_other')
    assert.equal(await evaluate(`document.querySelector('${context}').value`), 'Authored hierarchy draft')
    assert.equal(await evaluate('fixture.bookings.length'), 0)
    observations.push('authored-card-wrapper-keeps-active-payment-controls-visible')
    ...
    for (const lifetime of ['reopen', 'reinstall', 'pending-replacement', 'legacy-owner']) {
      await navigate(lifetime === 'legacy-owner' ? 'legacy-owner=1' : '')
      await openDetails('paid')
      await fill(context, 'Original A message')
      await fill(guests, 'original-a@example.invalid')
      await openPayment()
      await chooseCard('pm_other')
      await evaluate(`fixture.loseBookingResponse = true; ${lifetime === 'pending-replacement' ? 'fixture.pauseBookingResponse = true' : ''}`)
      await click(confirm)
      await waitFor(lifetime === 'pending-replacement' ? 'fixture.waiting' : 'fixture.bookings.length === 1')
      const original = await evaluate(`fixture.requests.find(r => r.path.endsWith(fixtureApi.BOOKING_PATH)).body`)
      await click('#close')
      if (lifetime === 'pending-replacement') {
        await click('#paid')
        await waitFor(`document.querySelectorAll('[data-paid-calendar-slot]').length > 1`)
        await evaluate(`document.querySelectorAll('[data-paid-calendar-slot]')[1].click()`)
        await click(confirm)
        await fill(context, 'Replacement B message')
        await fill(guests, 'replacement-b@example.invalid')
        await openPayment()
        await chooseCard('pm_original')
        await evaluate("fixture.pausePath = ''; fixture.release()")
        await waitFor('fixture.bookings.length === 1')
        await click(confirm)
      } else {
        await evaluate("fixture.defaultCard = 'pm_original'")
        if (lifetime === 'reinstall') await evaluate(`fixtureApi.installPaidBookingController({...fixtureSettings(true), starterName:'Changed display name', config:{...fixtureConfig(true),price_cents:50000}})`)
        await click('#paid')
      }
      await waitFor(`!!document.querySelector('[data-booking-recovery-check]')`)
      const recoveryText = await evaluate(`document.querySelector('[data-booking-recovery]').textContent`)
      assert.match(recoveryText, /Original A message/)
      assert.match(recoveryText, /original-a@example.invalid/)
      assert.match(recoveryText, /mastercard ending in 0042/)
      assert.match(recoveryText, /\$250/)
      assert.ok(recoveryText.includes(original.timezone))
      assert.ok(recoveryText.includes(new Intl.DateTimeFormat('en-US', {dateStyle:'long',timeStyle:'short',timeZone:original.timezone}).format(new Date(original.start))), 'recovery displays the original slot')
      assert.doesNotMatch(recoveryText, /Replacement B|replacement-b|Changed display name/)
      assert.equal(await evaluate('fixture.bookings.length'), 1, 'reopening and replacing details never sends request B')
      await click('#close')
      await openDetails('free')
      await fill(context, 'Independent Free Call')
      await click(confirm)
      await waitFor('fixture.bookings.length === 2')
      assert.equal(await evaluate('fixture.bookings[1].config_id'), 'fixture-free', 'Free Calls remain usable while Paid recovery is retained')
      await click('#close')
      await click('#paid')
      await waitFor(`!!document.querySelector('[data-booking-recovery-check]')`)
      await evaluate('fixture.rejectBookingStatus = 401')
      await click('[data-booking-recovery-check] button')
      await waitFor('fixture.bookings.length === 3')
      assert.match(await evaluate(`document.querySelector('[data-booking-recovery] [role="status"]').textContent`), /could not confirm/)
      assert.deepEqual(await evaluate('fixture.bookings[2]'), original)
      await click('[data-booking-recovery-check] button')
      await waitFor('fixture.bookings.length === 4')
      assert.deepEqual(await evaluate('fixture.bookings[3]'), original, 'recovery reuses the original slot, message, guests, card and key')
      assert.equal(await evaluate('fixture.canonicalBookings.size'), 2, 'only original Paid and independent Free bookings exist')
      assert.equal(await evaluate(`document.querySelector('[paid-call-text]').textContent`), 'Your card ending in 0042 will be used for this call.')
      assert.equal(await evaluate(`document.querySelector('[schedule-step="success"] [booking-element="context"]').textContent`), 'Original A message')
      assert.equal(await evaluate(`document.querySelector('[schedule-step="success"] [booking-element="price"]').textContent`), '$250')
      await click('#close')
      await openDetails('paid')
      assert.equal(await evaluate(`!!document.querySelector('[data-booking-recovery]')`), false, 'acknowledged recovery allows a fresh Paid review')
      observations.push(lifetime + '-recovers-original-paid-and-preserves-free-calls')
    }
    ...
    await click('[data-booking-recovery-check] button')
    await waitFor(`document.querySelector('[data-booking-recovery-check] button').getAttribute('aria-label') === 'Choose a new time'`)
    await click('[data-booking-recovery-check] button')
    await waitFor(`!!document.querySelector('[data-paid-calendar-slot]')`)
    assert.equal(await evaluate('fixture.bookings.length'), 2, 'terminal recovery returns to choosing a slot without submitting another booking')
    observations.push('reopened-terminal-failure-allows-new-paid-review')
