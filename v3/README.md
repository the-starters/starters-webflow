...
they pass through unchanged with the TalkJS session owner's own bearer on
non-inert surfaces. It installs on these exact staging paths:
...
both synchronous scripts: all scheduling-group requests, including TalkJS,
return HTTP `410` without installing authentication, discovery overrides, or
booking identity. The
