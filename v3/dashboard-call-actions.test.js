...
test('reschedule receipts show the selected slot without moving a confirmed booking', async () => {
  ...
  try {
    ...
    assert.equal(mounts[0].config.mode, undefined, 'pending edit keeps ordinary availability')
    assert.equal(mounts[0].now, undefined)
    ...
    for (const role of ['starter', 'brand']) {
      ...
      assert.equal(mounts[mounts.length - 1].config.mode, 'confirmed_reschedule')
      assert.equal(mounts[mounts.length - 1].config.booking_id, currentId)
      assert.ok(Number.isFinite(mounts[mounts.length - 1].now()))
