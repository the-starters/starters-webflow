...
;(function (global) {
  ...
  function smartFillTarget(documentObject, category) {
    ...
    var matching = Array.prototype.find.call(candidates, function (field) {
      return normalizedName(field.getAttribute && field.getAttribute('data-sp-fill-category')) === normalized
    })
    if (matching) return matching
    // Hire service cards are generated by browser code, while Webflow owns the
    // modal's native Services select. Use that exact owning form only when the
    // Designer has not supplied a tagged service target.
    return normalized === 'service' && form ? namedField(form, 'Services') : null
  }
