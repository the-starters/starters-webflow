...
test('continues Paid settings hydration when site Memberstack readiness never settles', async () => {
  const result = load({
    cardMode: true,
...
test('reconciles the Paid Build intent when site Memberstack readiness settles after timeout', async () => {
  const memberReady = deferred()
  const memberJSON = {}
  const result = load({
    cardMode: true,
