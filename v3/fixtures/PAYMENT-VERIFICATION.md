...
[booking-recovery-runtime.json](booking-recovery-runtime.json) records this run and the current deployment-candidate SHA-256. The runner remains `node v3/browser-tests/booking-recovery.xano.mjs --run`, with `PAID_CALL_XANO_BASE`, `PAID_CALL_HARNESS_KEY`, `PAID_CALL_WORKSPACE_APPROVED=paid-call-card-selection-test-20260915`, and a worktree-local `PAID_CALL_RECOVERY_EVIDENCE` path. Keep the harness key out of logs and source.
