...
### Review regression verification — 2026-09-16

The changed booking endpoint was installed alone in isolated workspace **6/v1** after a one-endpoint preview. Readback differed only by moving the future-start precondition into the fresh-booking branch. The CLI emitted unresolved-reference warnings during the scoped import; source readback retained those references and the subsequent real-HTTP checks passed. Direct workspace push was temporarily enabled for this approved test install and returned to **disabled** before runtime execution. Production workspace 2 and the existing harness secrets were unchanged.

`v3/browser-tests/booking-recovery.xano.mjs --run`, using the existing isolated harness environment variables and synthetic controls, passed four groups: past fresh-request rejection before command creation; completed replay after start with the original card; past reconciliation after a default-card change; and retained fingerprint, actor, Origin, authentication, and command/booking environment guards. This used synthetic canonical rows and command records; it did not create a provider booking or charge. The prior ten-group provider simulation was not rerun. The evidence file below now records the subsequent R8 rerun.

The focused Free/Paid Node tests passed **140 tests**. The booking browser fixture also passed, including visibility against the actual `card-setup-owner.html` dialog and recovery after older Free-owner initialization. The older-owner unit regression additionally verifies that adoption preserves an already active Free command. No Git push, PR, merge, tag, or frontend release was performed by this review phase.


### R8 canonical recovery verification — 2026-09-16

The changed booking candidate and test-only [recovery control](../browser-tests/booking-recovery-control.xs) were installed in isolated **workspace 6/v1**. The control adds configuration patching scoped to the authenticated synthetic run. It belongs only to the isolated harness, outside the deployment candidates. Import the booking candidate with the existing synthetic Nylas URL substitution and preserve workspace-specific GUIDs. Never install this control in production.

The initial test stopped in fixture setup because the importer converted the seed response object into text. Parenthesizing its filtered card value fixed the import; readback confirmed the seed response remained an object. The subsequent focused run passed **seven groups** through real authenticated HTTP, including original-price and original-duration recovery after service changes and booking acceptance, rejection of foreign actor/customer/card/slot snapshots, and fresh-request duration, price and active-service validation. Final inspection found one canonical booking and one command for the successful test run; recovery created no replacement booking.

[booking-recovery-runtime.json](booking-recovery-runtime.json) records this run and the SHA-256 of the candidate tested in that run. Subsequent local trailing-whitespace cleanup changes source bytes only; the recorded hash remains historical runtime evidence and has not been regenerated. The runner remains `node v3/browser-tests/booking-recovery.xano.mjs --run`, with `PAID_CALL_XANO_BASE`, `PAID_CALL_HARNESS_KEY`, `PAID_CALL_WORKSPACE_APPROVED=paid-call-card-selection-test-20260915`, and a worktree-local `PAID_CALL_RECOVERY_EVIDENCE` path. Keep the harness key out of logs and source.

Scoped-import unresolved-reference warnings were followed by source readback and the passing runtime test. Workspace 6 direct push was restored to disabled before execution. Synthetic records and existing secrets were preserved. No production changes, real provider mutations, frontend tests, full suites, Git delivery or release actions were performed in this fix round.
