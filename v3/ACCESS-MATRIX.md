...
>
> Paid-Brand completion comes from the authenticated Xano
> `starters_onboarding/get_brand_profile_status` response. The
> `thestarters:v3-brand-profile-completed` session marker, written only after a
> durable submit, short-circuits that read. The controller still writes the
> `completed-brand-profile` Memberstack field for endpoint #1513, but browser
> routing does not read it. This single-signal rule prevents the inbound and
> outbound redirects from disagreeing while the webhook mirror catches up.
> Unfinished, missing, malformed, failed, and timed-out Xano results all stay.
> Free-Brand and Talent branches remain zero-network. Unmapped, cross-role
> conflicted, and logged-out visitors are untouched, so the module never
> contradicts the gated group's decision. Wiring:
...
| Canonical `/dashboard` role routing and direct protected-page access | `v3/route-guard.js` | Implemented locally; install per [ROUTE-GUARD-WIRING.md](../docs/wiring/ROUTE-GUARD-WIRING.md); staging matrix pending |
...
| Logged-in role routing on `/complete-profile` | `v3/complete-profile-redirect.js` (page-scoped; Xano `get_brand_profile_status` plus the durable-submit session marker for paid Brands, and the guard's `memberRole`/`roleHome` for role destinations) | Implemented 2026-08-03, role branches added that evening, and paid-Brand completion moved to the same Xano signal as the inbound redirect on 2026-08-06; see [BRAND-ACCOUNT-WIRING.md](../docs/wiring/BRAND-ACCOUNT-WIRING.md) and [COMPLETE-PROFILE-REDIRECT-WIRING.md](../docs/wiring/COMPLETE-PROFILE-REDIRECT-WIRING.md). Paid-Brand results fail open by staying; free-Brand and Talent branches remain zero-network. Access control stays with the gated group in the row above |
| Talent funnel position on `/build-profile/*` | `v3/build-profile-redirect.js` (page-scoped, Xano `get_build_profile_status`) | Implemented 2026-08-03, migrated off the row-exists signal 2026-08-04; needs three page-level Webflow embeds, see [BUILD-PROFILE-REDIRECT-WIRING.md](../docs/wiring/BUILD-PROFILE-REDIRECT-WIRING.md) |
| Talent funnel position at login | `v3/auth-route.js` (Xano `get_build_profile_status`, Talent only) | Live since 2026-07-31, migrated off the row-exists signal 2026-08-04; the `/login`, `/starter-login`, and `/auth-route` embeds are pinned page-level code and must move together, see [AUTH-ROUTE-WIRING.md](../docs/wiring/AUTH-ROUTE-WIRING.md) |
...
| `/hire/<slug>` paid CTAs (Hire, Message, Book Call, call, service and rate cards) | `v3/hire-profile.js` `isPaywalledViewer` / `isBrandMember` and `v3/messages-profile.js`, opening the page's `signup-modal` whose form and upsell block are Memberstack `data-ms-content` gated | Decided 2026-09-21: outside the guard tables, the page stays public. Paid Brands get booking, contract and chat; every other signed-in non-owner, non-Talent viewer (free Brand, legacy Brand, unmapped plan) gets the signup modal, rendered like a logged-out visitor. See [HIRE-PROFILE-WIRING.md](../docs/wiring/HIRE-PROFILE-WIRING.md#paywalled-viewers) |
