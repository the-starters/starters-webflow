...
- `v3/brand-account-controller.js` — Memberstack-first Brand Build Account controller for the native `/complete-profile` form, plus an explicitly gated Account Security email controller. It validates the authored fields, prevents Webflow's native form request, writes `free-user`/`last-name`/`company`, changes login email only when needed, requests a reset/set-password email for the final normalized address, and stamps `completed-brand-profile` last so an interrupted update never falsely completes onboarding. Successful password-token redemption is the ownership proof; the controller does not claim Memberstack `verified=true` without separately observed state. Transient profile/auth assignments retry once; the email request is not retried; duplicate submits collapse; Account Security interception remains off until its competing native submit owner is intentionally removed. Xano remains downstream of the existing Memberstack webhook (#1513), keyed by stable Memberstack member ID. `complete-profile-photo.js` now binds the same page's authored upload control to Memberstack's supported profile-image action instead of calling the Starter-only Xano upload endpoint. Install order, native markup contract, configuration switches, and canaries live in [`v3/BRAND-ACCOUNT-WIRING.md`](v3/BRAND-ACCOUNT-WIRING.md)
