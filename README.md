...
- `v3/brand-account-controller.js` — Memberstack-first Brand Build Account controller for the native `/complete-profile` form, plus a configuration-gated Account Security email controller. Xano remains downstream of Memberstack webhook #1513, keyed by stable member ID, and `complete-profile-photo.js` binds the authored upload control to Memberstack's profile-image action. The authoritative contract and canaries live in [`v3/BRAND-ACCOUNT-WIRING.md`](v3/BRAND-ACCOUNT-WIRING.md)
