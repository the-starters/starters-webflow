...
- `v3/starter-dashboard-points.js` — authenticated, attribute-driven `/starter-dashboard` points and rank tile; renders Xano-canonical rank and cohort values as compact ordinal positions, normalizes the existing authored subline rows in place, keeps legacy tie labels hidden while retaining Xano tie semantics, and leaves Webflow owning markup, links, styling, and unavailable-rank state cards
