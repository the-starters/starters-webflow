...
- `scripts/learn-reindex.mjs` — the Node tool that rebuilds the `LearnContent` Algolia index from the Webflow Learn collections (Interview & News, Playbooks & Frameworks, Sessions, Webinars, Events), so what people find in Learn search matches what is actually published. None of it is served to a browser or reachable from a CDN script; the mapping lives in `scripts/learn-reindex-lib.mjs` and is covered by `scripts/learn-reindex.test.mjs`. `node --env-file=../staging-qa/.env scripts/learn-reindex.mjs --dry-run` prints what would change and writes nothing; add `--webflow-export <file>` to read the CMS from a JSON export instead of the Webflow API. `--write` replaces the whole index in one swap, so every Learn card and search result changes at once — always read a dry run first
