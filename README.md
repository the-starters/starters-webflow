...
- `v3/route-guard.js` — V3-only shared stable role resolver, canonical `/dashboard` router, and direct-access guard for protected role-scoped pages; also bounces an already-signed-in member off the four public entry pages (`/`, `/login`, `/starter-login`, `/sign-up`) to a validated `?next=` or their role home while leaving signed-out visitors completely untouched, and supports per-page logged-out destination overrides so the build-profile funnel and `/complete-profile` send a stranger to the homepage instead of a login form
