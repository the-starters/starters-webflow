...
- `v3/reviews.js` — prepared V3 review-form and public-profile adapter; Webflow owns the authored markup and wf-xano transport while Xano owns eligibility, moderation, aggregates, and points outcomes. Production activation is paused; see [`v3/README.md`](v3/README.md#v3-reviews-frontend) for the authoritative wiring contract
...
- `v3/complete-profile-redirect.js` — `/complete-profile` logged-in role routing: puts every **mapped** member who lands on the page where they actually belong, with **no hop through `/login`**. The page is a paid-Brand form, so a paid Brand is the only role that can stay: they stay until the Memberstack member custom field `completed-brand-profile` carries a real value and then get `location.replace('/brand-dashboard')`. A free Brand goes to its quiz-funnel home (`/quiz-results` once `starter-quiz` is set, else `/quiz`) and a Talent member to `/starter-dashboard` — both taken from the sitewide `v3/route-guard.js` contract (`memberRole` plus `roleHome`) rather than a second copy of the plan table or of `ROLE_DEFAULTS`, so those are by construction the same destinations the guard's member-home bounce would have produced after a `/login` round trip, minus the round trip and the login form flashing at an already-authenticated member. The completion field is written by a hidden Designer input (`data-ms-member="completed-brand-profile"`) on the form itself and is consulted for the paid-Brand branch only, so the whole decision reads off the member object Memberstack already returned — **no network request at all**, no Xano surface. A trimmed non-empty string (or any non-string truthy value, since checkbox-backed fields can come back boolean) means done; an absent, empty, or whitespace-only value means stay, the same truthiness rule `v3/route-guard.js` applies to the `starter-quiz` marker, and the same deliberate bias — re-showing a completed form is a harmless annoyance, sending a member away from a form they still have to fill in is not. Scoped to `/complete-profile` and `/complete-profile/` on the approved hosts. It is emphatically **not** the page's access gate and never sends anyone *to* the page: the Memberstack `restrict-pages` gated group (dashboard field values, no leading slash in either: URL rule STARTS `complete-profile`, Access Denied URL `login`, which lands a denied visitor on the path `/login`) remains the sole owner of access and of the logged-out kick, `route-guard.js` deliberately keeps the page out of `PAGE_ROLES`, and tests on both sides assert that layering. The role branches do assume one thing about that group — its access must be **All Members**, not the paid plan, or a logged-in free Brand or Talent member never reaches the page to be routed (decision 2026-08-03 evening, replacing the earlier paid-only design). Fail-open everywhere else — unmapped, cross-role conflicted, logged out, Memberstack missing or slow, a missing or half-loaded role contract (a guard that can name a role but not its home counts as no contract at all), or a member lookup that throws all leave the page exactly as authored. Two deliberate properties: the paid-Brand branch is inert until the hidden input starts writing the field and members who completed the form before the field existed read as not-done until they resubmit once (there is no backfill), while the free-Brand and Talent branches need no new field and are live as soon as the embed is. Needs the hidden input plus one page-level embed installed after the guard; wiring, the Memberstack group contract, and QA order live in [`v3/COMPLETE-PROFILE-REDIRECT-WIRING.md`](v3/COMPLETE-PROFILE-REDIRECT-WIRING.md)
- `account-settings/plan-dates.js` — Memberstack plan/billing dates as text: writes the resolved date into every `[ms-form-pause-date]` element, formatted `Jan 10, 2000`. Five field names — `signup` (`member.createdAt`), `last-billing`, `next-billing`, `cancel-at`, and `resumes-at` (`next-billing` plus `ms-form-pause-months` months, default 1) — with `ms-form-pause-format` (`medium`/`long`/`numeric`/`iso`), `ms-form-pause-empty`, `ms-form-pause-id`, and `ms-form-pause-tz` readable from the element or any ancestor wrapper, so one wrapper can configure a whole block. The pause length can also come from the member: mark a radio group `ms-form-pause-input` (on the wrapper once, or on each input) and `resumes-at` re-renders on every `change` via a delegated `document` listener, with no Memberstack round trip since only the arithmetic changed. Control values are parsed for their first integer so `2` and `2 months` both mean two, an unchecked radio expresses nothing and is skipped (reading it anyway is how a three-option group reports whichever option sits first in the DOM instead of the one picked), and radios, checkboxes, `<select>`, and a plain number input all work. Pause sources resolve nearest-first — a marked control in the closest ancestor that has one, **counting that ancestor itself**, then the document, then the static attribute, then one month — which is what lets two independent pause groups coexist on a page. Two properties of that walk are load-bearing and both fail the same silent way, looking correct on any page with only one group: the self check, because `querySelectorAll` is descendants-only and without it the walk skips the wrapper the author actually marked; and treating an all-unchecked group as the answer "nothing chosen" rather than as "no group here", because otherwise the walk continues to a common ancestor and reports a **sibling** group's checked option. `ms-form-pause-reveal` on a wrapper keeps the whole sentence — copy and inline date together — hidden until both a marked control has expressed a pause length and every date element inside resolved; the second half is the point of wrapping a sentence rather than a date, since a logged-out visitor, a free-plan member, or a failed lookup otherwise renders a visible "Billing resumes —". A static `ms-form-pause-months` does not count as choosing, and because "not chosen yet" and "chose one month" both compute one month, the reveal test reads the source of the answer rather than its value. Hiding is a marker class (`is-ms-form-pause-shown`) paired with a one-line CSS rule the author pastes, not an inline style: the rule hides the block from first paint so nothing flashes before this deferred script runs, revealing need not guess whether the Designer set `block`/`flex`/`grid`, and a script that never loads leaves the sentence hidden, which is correct for copy whose only content is a date it cannot fill. Deliberately not the `hidden` attribute, which a Webflow class carrying `display: flex` overrides. `resumes-at` is anchored on the paid-through date and never on signup, because a member pausing on the 20th of a cycle that renews on the 1st would otherwise ride 11 unpaid days or be charged mid-pause; that matches Stripe's own `pause_collection.resumes_at`, which is what has to perform the actual pause. Reads only — pausing needs the Stripe secret key and belongs behind a Xano endpoint, so this module is safe to install before that endpoint exists. Three deliberate properties: timestamps are unit-sniffed (Date, ISO string, Unix seconds, or Unix milliseconds, split at 1e10) because `createdAt` arrives as ISO while `payment.*` arrive as numbers and a seconds value through `new Date()` renders in January 1970; formatting defaults to UTC so the printed day equals the billing day for viewers west of UTC; and month arithmetic clamps the day, so Jan 31 + 1 month is Feb 28 (Feb 29 in a leap year) and never Mar 3. The plan connection is auto-picked as the first active one carrying a `payment` object rather than the first listed, since a member can hold both a free and a paid connection and only the paid one has billing dates. Fail-quiet everywhere: a logged-out visitor, a free-only member, a `payment: null` connection, a pinned plan id that is absent, a failed member lookup, or Memberstack never loading all render the `ms-form-pause-empty` text (default `—`) instead of a stale or `Invalid Date` value. `payment.cancelAtDate` is the one field name not confirmed against Memberstack's published response example, so `cancel-at` renders the empty text where that key is absent and should be checked against a real cancel-at-period-end member before shipping UI that depends on it. Re-renders on `onAuthChange` and on a bfcache restore; diagnostics are staging-only
