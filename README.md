...
- `navbar-embeds/nav-menu.js` — mobile menu button (`#menu-btn`) toggles body scroll lock. Clears Webflow placeholder `srcset`/`sizes` on `img[nav-profile-image][data-ms-member="profile-image"]` at Webflow startup, including desktop pages without the menu button. Memberstack retains ownership of `src`; no member data is fetched or written. This shared navbar compatibility fix prevents responsive placeholder candidates from overriding an already-synced member photo after reload.
