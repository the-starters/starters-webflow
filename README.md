...
- `v3/agency-profile.js` — `/hire/<slug>` Agency section: agency name, team size, contract type, average project size, and the intro video, read from the public Xano endpoint `profile/starter/agency/v1`. `wf-xano` owns the fetch, the text binds, and every show/hide decision; this script only supplies the profile slug the URL path carries (which `wf-xano` cannot source), sets the video iframe's `src` (which `wf-xano` has no documented binder for), and owns the section wrapper's hidden state so a hidden section stops consuming its parent's flex/grid gap. Because the authored loading spinner lives inside that wrapper, the wrapper stays visible for the length of the request — so every profile, agency or not, briefly shows the spinner before the section either renders or collapses; keep the spinner short so the settle shifts little. The section fails closed in every direction (no slug, no instance, a duplicated instance key, a throw inside the library, or a failed request all collapse it) and fails open on one: an 8-second cap collapses the section when a request never settles, since `wf-xano` sets no fetch timeout of its own. The authoritative attribute contract, markup, install, and verification live in [`v3/AGENCY-PROFILE-WIRING.md`](v3/AGENCY-PROFILE-WIRING.md)
...
- `v3/dashboard-call-actions.js` — native dashboard booking-action binder; see
  `v3/README.md` for the authoritative decline, cancel, and reschedule boundary
