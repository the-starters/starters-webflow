...
- `v3/hire-profile-taxonomy.css` — `/hire/<slug>` taxonomy-group empty-state rule; hides an authored `[xwf-empty-check]` group until its wf-xano list renders a `[wf-xano-item]`, and loads in the hire template **Head** so an empty group never flashes visible; the authoritative marker placement, install, and rationale live in [`v3/HIRE-PROFILE-WIRING.md`](v3/HIRE-PROFILE-WIRING.md#taxonomy-empty-groups-stay-hidden)
