...
- `v3/auth-route.js` — V3-only login/signup router consuming the sitewide route guard's stable role contract, with role-scoped `next` destinations; canonical `next=/dashboard` resolves directly to the role home
...
- `v3/route-guard.js` — V3-only shared stable role resolver, canonical `/dashboard` router, and direct-access guard for protected role-scoped pages
...
- `v3/starter-dashboard-stripe-connect.js` — Memberstack-scoped V3 Stripe Connect dashboard and OAuth-callback controller; paints loading/disconnected/incomplete/ready/review/error states from the Xano-native Stripe mirror, enables the external Stripe Earnings link only when `charges_enabled` is true, starts OAuth or onboarding through Xano, rejects cross-member callback state, and re-reads canonical status after the return instead of waiting on the V2 nightly CMS batch
...
- `v3/onboarding-done-redirect.js` — `/starter-onboarding` read half of the onboarding-completion pair: trades the Memberstack JWT at `api:g1vmSLWh/auth/trade-token/v3` for a Xano token, reads `starters_onboarding/get_freelancers` (envelope `{"freelancer":[<one record>]}`), and calls `location.replace('/starter-dashboard')` only on a literal `onboarding_done === true`. Fail-open everywhere: a string `"true"`, a `1`, a missing field, an empty envelope, a logged-out visitor, a failed or empty token trade, a non-2xx read, or an 8-second timeout all render the page exactly as authored, because the redirect is a UX courtesy and never a security boundary — access control stays with Memberstack, `v3/route-guard.js`, and Xano endpoint authorization. Also consumes `sessionStorage['starter-onboarding-just-submitted']` on every load (reads it, removes it, skips the redirect check exactly once), so the post-submit completion view survives the beat between the write and the read, and an abandoned submit can never suppress more than one load. Installs as a pinned pair with `v3/patch-onboarding-status.js` on that page only — never ship one without the other; wiring, QA order, and troubleshooting live in [`v3/ONBOARDING-DONE-REDIRECT-WIRING.md`](v3/ONBOARDING-DONE-REDIRECT-WIRING.md)
- `v3/patch-onboarding-status.js` — `/starter-onboarding` write half of the same pair: one `MutationObserver` per `.w-form` wrapper on its `.w-form-done` sibling (`attributeFilter: ['style', 'class']`, an inline display that is neither empty nor `none` as the signal, `offsetParent` as the fallback for a class-driven done state), so completion is detected from Webflow's own success state rather than from a submit click — the page's step controller owns the buttons and a click only means "tried". Both native forms count, full profile and consult; the first positive mutation disconnects that wrapper's observer and marks the wrapper so it can never double-fire, and a done block already visible at boot is left alone. On success it writes `sessionStorage['starter-onboarding-just-submitted']` **before** the network call, so the completion view survives even if the write never lands, then `PATCH`es `starters_onboarding/set_onboarding_status` (bearer, no body, answers `{"onboarding_done": true}`) through the same trade-token auth and 8-second budget, retrying at roughly 1s and 3s with a fresh token trade between failed attempts before giving up silently behind a staging-only warning — a missed mark self-heals on a later visit, so this never blocks or reloads the page. `markOnboardingDone()` on `window.StartersPatchOnboardingStatus` is callable by hand on staging to exercise the whole write without submitting a form. Installs as a pinned pair with `v3/onboarding-done-redirect.js`; see [`v3/ONBOARDING-PATCH-STATUS-WIRING.md`](v3/ONBOARDING-PATCH-STATUS-WIRING.md)
