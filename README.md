...
- `v3/talkjs-auth-session.js` — shared signed TalkJS Classic session owner loaded by the V2/V3 Messages clients; trades the Memberstack session for a `user_v3` bearer, requests a short-lived TalkJS user token from `talkjs/user-token/v3`, and asks `talkjs/conversation/v3` to authorize or provision two-person conversations (browser conversation sync is off)
