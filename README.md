...
- `utils/workflow-diagnostics.js` — sitewide privacy-safe receipt helper for important V3 forms. It stores and copies only allowlisted workflow metadata: diagnostic ID, UTC time, controller/environment, result/stage, safe error code, HTTP status, duration, request-attempted state, canonical record type/ID, and replay state. It never accepts form answers, names, emails, prices, tokens, request/response bodies, or idempotency keys. The covered controllers load the helper from the same versioned jsDelivr release before a mutation starts.
