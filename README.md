...
- `quiz-main/quiz-home.js` — Home-only controller; saves selected category bucket IDs to `sessionStorage.quizSelectedCategories`, redirects to `/quiz`, and replaces the Home consult rail's legacy `also-worked-with` company text with ordered Algolia `work-history` names joined by the stable profile ID read from the `freelancer-{id}` CMS image filename, taking its credentials and Starter index only from the managed host resolution in `v3/algolia-environment.js` (so the Home tag must load after it; see `quiz-main/README.md`)
