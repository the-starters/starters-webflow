...
- `quiz-main/quiz-home.js` — Home-only controller; saves selected category bucket IDs to `sessionStorage.quizSelectedCategories`, redirects to `/quiz`, and replaces the Home consult rail's legacy `also-worked-with` company text with ordered Algolia `work-history` names looked up by the stable profile ID read from the `freelancer-{id}` CMS image filename, taking its credentials and Starter index only from the managed host resolution in `v3/algolia-environment.js` (so the Home tag must load after it; see `quiz-main/README.md`)
...
- `opportunities-3.0.js` — Opportunities 3.0 page and dashboard binder (including the role-gated merged `/opportunities` feed plus category-matched and applied starter feeds); binds the paid-Brand create/edit forms, validates their custom category selector, keeps the authored 15-word opportunity-title rule while adding a native 120-character backstop, maps ongoing Part Time estimated weekly hours through the existing Xano contract, paints the authored create/edit success screen with the saved opportunity title and opportunity-specific copy, defers access decisions to the sitewide `v3/route-guard.js` when present, redirects a foreign brand off an opportunity it does not own to the merged `/opportunities` feed, drives authenticated canonical project actions on both role dashboards, and keeps invoice generation and cancellation Starter-only while using the shared diagnostic receipt contract below; the shared opportunity detail page's role-scoped wf-xano feed activation and its ordered release live in [`docs/wiring/OPPORTUNITY-DETAIL-ROLE-FEEDS-WIRING.md`](docs/wiring/OPPORTUNITY-DETAIL-ROLE-FEEDS-WIRING.md)
...
- `v3/starter-profile-claim.js` — fail-closed premade Starter profile claim gate for exact `/hire/<slug>` QR links; it scrubs the raw capability before analytics, exchanges it through the exact prepare endpoint, and reveals the authored signup popup only after receiving an exact-path one-use exchange code. The authoritative markup, backend, install-order, and release contract lives in [`docs/wiring/STARTER-PROFILE-CLAIM-WIRING.md`](docs/wiring/STARTER-PROFILE-CLAIM-WIRING.md)
...
- `v3/hire-profile-taxonomy.css` — `/hire/<slug>` taxonomy-group empty states; behavior, marker placement, and installation live in [`docs/wiring/HIRE-PROFILE-WIRING.md`](docs/wiring/HIRE-PROFILE-WIRING.md#taxonomy-empty-groups-stay-hidden)
...
- `v3/profile-form/` — shared Build/Edit Profile inline-extraction candidates plus the whole-dollar price contract the Build and Edit Profile forms enforce on hourly, monthly retainer, and custom-service prices before any Xano request, and on the Paid Call rate — on Edit Profile before the Dashboard-owned canonical write, and on Build Profile before its private Call Settings intent receipt is stored; the authoritative ownership, provenance, page-Head-Code cutover, loader order, verification, and [price ranges and rejection rules](v3/profile-form/README.md#whole-dollar-price-contract) live in [`v3/profile-form/README.md`](v3/profile-form/README.md)
...
- `v3/build-profile/submit-writer.js` — Build Profile native-form submit writer and normalized payload controller; it never sends call fields to the profile endpoint and instead stores the visible Free and Paid choices as a private member-bound Call Settings intent receipt that Dashboard and Edit Profile materialize
...
- `v3/starter-edit-profile/profile-section-validation.js` — section-scoped Edit Profile field validation (`window.StarterProfileValidation`), derived from the sitewide `utils/wf-validate.js` but scoped to one section's own fields and installing no submit gate; the globals, load order, authored markers, and `form-xano-required` contract live in [`v3/starter-edit-profile/README.md`](v3/starter-edit-profile/README.md#unified-rows-services-work-experience-highlights)
- `v3/starter-edit-profile/unified-services.js` — Edit Profile Services & Rates row coordinator (`window.StarterProfileSections`); it owns the rows, draft state, and Save gating, and `starter-edit-profile.js` stays the writer
- `v3/starter-edit-profile/unified-companies.js` — Edit Profile Work Experience row coordinator (`window.StarterProfileCompanies`); `company-experience-crud.js` binds it and stays the writer
- `v3/starter-edit-profile/unified-highlights.js` — Edit Profile Highlights row coordinator (`window.StarterProfileHighlights`); `portfolio-crud.js` binds it and stays the writer
...
- `v3/brand-account-controller.js` — Memberstack-first Brand signup and Build Account controller plus configuration-gated login-email ownership for legacy Account Security forms without a native `data-ms-form="profile"` or `"email"` marker (the published form keeps native Memberstack ownership) and the visible Talent edit-profile form. Its owned mutations emit the shared privacy-safe success, validation, timeout, HTTP, and password-email receipts without recording identity fields. The authoritative signup mapping, identity scope, propagation contract, failure semantics, and reversible canaries live in [`docs/wiring/BRAND-ACCOUNT-WIRING.md`](docs/wiring/BRAND-ACCOUNT-WIRING.md)
...
- `v3/dashboard-call-actions.js` — native dashboard booking-action and details
  dialog navigation binder; see `v3/README.md` for the authoritative decline,
...
- `v3/paid-call-settings.js` — native Starter paid-call settings controller backed only by canonical Xano get/upsert/disable endpoints; it also hydrates and consumes member-bound Build Profile intent after Calendar, Availability, and Stripe are ready; the authoritative Designer wiring, authority, and release gate live in [`docs/wiring/PAID-CALL-SETTINGS-WIRING.md`](docs/wiring/PAID-CALL-SETTINGS-WIRING.md)
- `v3/free-call-settings.js` — native Starter free-call settings controller for the published Call Free Form, fixed at 30 minutes and $0 and backed only by canonical Xano get/upsert/disable endpoints; it also hydrates and consumes member-bound Build Profile intent after Calendar and Availability are ready; the authoritative Designer wiring, painted state contract, authority, and release gate live in [`docs/wiring/FREE-CALL-SETTINGS-WIRING.md`](docs/wiring/FREE-CALL-SETTINGS-WIRING.md)
...
- `v3/messages.js` — Memberstack + TalkJS inbox bootstrap for `/messages`, using the shared signed-session owner below; see [`v3/README.md`](v3/README.md#brand-and-starter-dashboard-messages-tile) for its existing-conversation and member deep-link contracts
- `v3/talkjs-auth-session.js` — shared signed TalkJS session and server-authorized conversation owner for the V3 Messages inbox, profile chat, and dashboard tile; the authoritative identity, retry, and mutation boundary lives in [`v3/README.md`](v3/README.md#shared-talkjs-authentication)
...
- `v3/brand-project-proposals.js` — V3 Brand Dashboard accept/decline controller for pending Starter project requests; a Starter submission only becomes a canonical project and a PandaDoc invitation once a Brand approves it here. See [`docs/wiring/BRAND-PROJECT-PROPOSALS-WIRING.md`](docs/wiring/BRAND-PROJECT-PROPOSALS-WIRING.md) for the endpoint, Designer, install, and release contract. Install it in the same release as `v3/starter-project-form.js`
...
- `navbar-embeds/memberstack/ms-code-field-link.js` — binds placeholder `[ms-code-field-link]` elements to member-specific destinations, including navbars added after initial page startup. Other fields keep their external-link behavior; `freelancer-profile-url` instead resolves the current member through Xano to the internal `/hire/<slug>` route and never reads the legacy V2 field value. From the first paint, the unresolved link stays visible as `View Profile (Publishing)`, applies the class named by `data-class-disabled`, has no navigation attributes, and polls until it can repaint as the active profile link.
...
- `navbar-embeds/navbar-explore/view-all.js` — "view all" button routes through the hidden View All Map in the navbar (`[data-view-all-map] [data-view-all-path]` matched on the row's full `wf-algolia-value` path → that entry's href); falls back to `/subcategories/<slug>` derived from the label when the map is missing or has no match, warning once on staging
...
- `utils/posthog-track.js` — the shared `StartersTrack.track(name, props)` funnel-event helper: stamps a consistent `platform` (`v2` / `v3`) property and makes a missing or blocked PostHog unable to break page logic. It also captures uncaught errors and unhandled promise rejections; object rejection reasons expose only bounded `message`, `code`, `status`, and `name` diagnostics so arbitrary request or member data is not sent. Browser source filename, line, and column are retained when available; detail-free cross-origin errors are dropped. On the three V3 hosts it allowlists (`thestarters.com`, `www.thestarters.com`, `the-starters-3-0.webflow.io`; `window.STARTERS_PLATFORM` does not widen that list), a confirmed chunk-load failure (a `ChunkLoadError`, or a `Loading chunk <id> failed` message) naming a `cdn.prod.website-files.com/.../js/webflow.*.js` asset is captured first and then gets one page-scoped recovery reload within a five-minute cooldown. Session storage holds a bounded list of per-pathname markers (page and timestamp only), so recovering one page never re-arms another page's cooldown; unavailable storage fails closed. Marker.io and all other providers remain capture-only. Its scoped PostHog `before_send` hook marks the root of forwarded exceptions as unhandled, preserves manual captures and nested causes, and runs existing hooks afterward so privacy filtering and event drops still apply. The snippet queue installs the hook before replaying captures. Event names and properties are defined in `platform-ops/architecture/posthog-funnel-events-plan.md`; renames need a migration note there
...
- `scripts/learn-reindex.mjs` — the Node tool that rebuilds the `LearnContent` Algolia index from the Webflow Learn collections (Interview & News, Playbooks & Frameworks, Sessions), so what people find in Learn search matches what is actually published. None of it is served to a browser or reachable from a CDN script; the mapping lives in `scripts/learn-reindex-lib.mjs` and is covered by `scripts/learn-reindex.test.mjs`. `node --env-file=../staging-qa/.env scripts/learn-reindex.mjs --dry-run` prints what would change and touches nothing in Algolia — every run, dry or not, also dumps the proposed records to the gitignored `scripts/.learn-reindex-out/`; add `--webflow-export <file>` to read the CMS from a JSON export instead of the Webflow API. `--write` replaces the whole index in one swap, so every Learn card and search result changes at once — always read a dry run first
...
unavailable, and when a project card renders before its canonical row.

Each Webflow-authored invoice row may include a
`[data-project-invoice-action="cancel"]` control inside the existing
`[wf-xano-element="nest-target"][wf-xano-field="invoices"]` target. The
controller never creates this control or a cancellation modal. It keeps the
control hidden until the exact `/starter-dashboard` role gate resolves and the
...
including when that promise rejects. After it settles, the guard performs a fresh
Memberstack identity read, aligns `window.MEMBER`, the guard member ID, authored
callback arguments, and the physical draft key to that result, and then releases
authored callbacks. An anonymous or failed live read clears stale identity state
before callbacks run. This keeps every consumer on one member when a browser
changes sessions and fails closed when no current member can be proved.
