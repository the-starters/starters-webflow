...
- `v3/brand-account-controller.js` — Memberstack-first Brand signup, Build Account, and role-scoped Account Security controller. It assigns the Test Brand plan to the native signup form on the Webflow staging hostname and preserves the live Brand Free plan on production, without replacing Designer-authored markup. It checks the canonical route-guard role contract at every account submit, makes account mutations durable before attempting one Memberstack reset/set-password email, and never automatically retries that non-idempotent email call. Xano remains downstream of Memberstack webhook #1513, keyed by stable member ID. The authoritative signup mapping, prerequisites, account contract, and canaries live in [`v3/BRAND-ACCOUNT-WIRING.md`](v3/BRAND-ACCOUNT-WIRING.md)
...
- `v3/dashboard-calls.js` — authenticated canonical call-section and Brand hero
  binder for the V3 dashboards. On `/brand-dashboard`, it also observes the
  existing Memberstack-native `form[data-ms-form="profile"]` without taking
  submit ownership, waits for `getCurrentMember()` to confirm the saved first
  name, last name, and company, then repaints the existing Designer-authored
  hero immediately. Failed or unconfirmed saves leave the hero unchanged.
  See `v3/README.md` for its Designer markup, identity, state, and endpoint
  contracts
