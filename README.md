...
- `v3/complete-profile-redirect.js` — `/complete-profile` paid-Brand completion redirect: keeps a paid Brand who already finished the Complete-profile form from re-entering it, and does nothing else. Completion is the Memberstack member custom field `completed-brand-profile`, written by a hidden Designer input (`data-ms-member="completed-brand-profile"`) on the form itself, so the whole check reads off the member object Memberstack already returned — **no network request at all**, no Xano surface. A trimmed non-empty string (or any non-string truthy value, since checkbox-backed fields can come back boolean) means done and calls `location.replace('/brand-dashboard')`; an absent, empty, or whitespace-only value means stay, the same truthiness rule `v3/route-guard.js` applies to the `starter-quiz` marker, and the same deliberate bias — re-showing a completed form is a harmless annoyance, sending a member away from a form they still have to fill in is not. Scoped to `/complete-profile` and `/complete-profile/` on the approved hosts, paid Brand only, with the role taken from the sitewide `v3/route-guard.js` contract rather than a second copy of the plan table. It is emphatically **not** the page's access gate and never sends anyone *to* the page: the Memberstack `restrict-pages` gated group (URL rule STARTS `complete-profile`, redirect `login`) is the sole owner of access, `route-guard.js` deliberately keeps the page out of `PAGE_ROLES`, and tests on both sides assert that layering. Fail-open everywhere — Talent, free Brand, unmapped, conflicted, logged out, Memberstack missing or slow, missing role contract, or a member lookup that throws all leave the page exactly as authored. Two deliberate properties: it is inert until the hidden input starts writing the field, and members who completed the form before the field existed read as not-done until they resubmit once (there is no backfill). Needs the hidden input plus one page-level embed installed after the guard; wiring and QA order live in [`v3/COMPLETE-PROFILE-REDIRECT-WIRING.md`](v3/COMPLETE-PROFILE-REDIRECT-WIRING.md)
