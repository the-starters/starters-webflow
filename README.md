...
- `v3/brand-account-controller.js` — Memberstack-first Brand Build Account controller for the native `/complete-profile` form, plus a role-scoped Account Security email controller. It checks the canonical route-guard role contract at every submit, replays non-Brand submissions to the native Memberstack handler, and routes reset/set-password delivery through a durable member-and-email-keyed issuer so retries resume without a separate verification message or duplicate email. Xano remains downstream of Memberstack webhook #1513, keyed by stable member ID, and `complete-profile-photo.js` binds the authored upload control to Memberstack's profile-image action. The authoritative contract and canaries live in [`v3/BRAND-ACCOUNT-WIRING.md`](v3/BRAND-ACCOUNT-WIRING.md)
