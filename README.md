...
`data-opp-status="closed"`; at runtime the script upgrades those legacy status
twins to a modal-local `data-close-opp-title="confirm|success"` contract. Opening
the modal always restores the confirmation title, and only a successful Close
mutation reveals the success title. This avoids treating the shared brand-list
modal as if it had one document-level opportunity status.
