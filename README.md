...
- `v3/brand-profile-redirect.js` — `/brand-dashboard` **inbound** half of the Brand profile-completion loop: raises `[data-page-spinner]`, trades Memberstack JWT at `api:g1vmSLWh/auth/trade-token/v3`, reads `starters_onboarding/get_brand_profile_status`, and `location.replace('/complete-profile')` on exactly one shape — literal `has_record === true` alongside literal `brand_profile_done === false` — leaving the spinner up through navigation. Same Xano signal as the outbound half and auth-route; marker `thestarters:v3-brand-profile-completed` short-circuits before any network call (this file never writes/clears it). Fail-open everywhere else. No role logic — Talent/free-Brand get `has_record: false` and stay; `v3/route-guard.js` owns `brand-paid` routing first. One deferred page-level tag on `/brand-dashboard` only, after the guard. Wiring + QA (marker round-trip is the critical step): [`v3/BRAND-PROFILE-REDIRECT-WIRING.md`](v3/BRAND-PROFILE-REDIRECT-WIRING.md)
