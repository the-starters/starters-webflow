...
- `v3/starter-dashboard-points.js` — authenticated, attribute-driven `/starter-dashboard` points and rank tile; renders the Xano-canonical ledger total plus overall and primary-role competition ranks, with safe loading, refreshing, ineligible, quarantined, missing-role, and error states, and falls back to clearing authored placeholders when the hero omits the loading/content/error wrappers
