...
- `global-embeds/form-embeds/memberstack-loader/memberstack-loader.js` — makes a Memberstack auth form's Button (login, signup, forgot-password, reset-password) look and behave busy and disabled while Memberstack shows its spinner, and restores it on hide; a successful submit keeps the button pending until Memberstack's redirect takes the page. Loader routing covers every Memberstack form, auth and profile alike: the submitting form's own Button Spinner is the one that turns. Two paths ship. A page that authors `data-ms-loader` keeps the attribute where it is, because Memberstack pins the element it cached at load, and the script mirrors that element's show and hide onto the submitting form's Spinner. A page that authors none gets the attribute placed on the submitting form's Spinner at submit time. A form with no Spinner leaves Memberstack to its full-screen overlay. A double-submit guard means repeat submits while the button is pending never reach Memberstack. The ownership marks `data-memberstack-loader-theme`, `data-memberstack-loader-busy` and `data-memberstack-loader-aria` are written at runtime and must never be authored in Webflow; only marked attributes are ever removed, so a peer script's hold on the same CTA survives. Call `window.startersMemberstackLoader.rescan()` for forms added after load, and read `window.startersMemberstackLoader.release` for the version in the page. Load it site-wide from the same global embed as password-validation, pinned to a version tag. Designer contract: put `data-ms-loader` on the Button Spinner; a missing Spinner degrades to Memberstack's overlay, and the warning about that is one of the [staging-only console diagnostics](#staging-only-console-diagnostics). Source: [memberstack-loader.js](global-embeds/form-embeds/memberstack-loader/memberstack-loader.js)
