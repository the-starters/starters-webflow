...
- `v3/auth-page-loader.js` — minimal V3 login and `/auth-route` runtime loader; preserves `route-guard.js` before `auth-route.js`, omits unrelated application controllers on auth pages, and completes the timestamp-only timing receipt on the final destination; see [`v3/AUTH-ROUTE-WIRING.md`](v3/AUTH-ROUTE-WIRING.md)
