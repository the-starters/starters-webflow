...
- `utils/workflow-diagnostics.js` — authoritative shared receipt contract for the Talent Application, opportunity create/edit/close/reopen, application apply/edit/withdraw/archive/restore, Generate Invoice, project lifecycle, project review, and Starter Edit Profile workflows. The controllers preserve the native Webflow surfaces and add the diagnostic ID and copy action to an existing success, error, or status message when one is available. The helper stores and copies only allowlisted metadata: diagnostic ID, UTC time, controller/environment, workflow result/stage, safe error code, HTTP status, duration, request-attempted state, canonical record type/ID, and replay state. Receipts exclude names, emails, phone numbers, form answers, prices, tokens, authorization headers, request/response bodies, and idempotency keys. The covered controllers load the helper from the same versioned jsDelivr release before a mutation starts. Use `copyWorkflowDiagnostic('<workflow>')` as the console fallback for the latest receipt in the current tab.
...
- `v3/talent-application-ui.js` — GitHub-owned replacement for the former 23 KB Webflow Code Embed on `/freelancer-application/step-1`; owns authored field validation, conditional blocks, location selects, and custom-select UI but never owns submission transport. After its release, replace the full legacy inline block with one deferred `@latest/v3/talent-application-ui.js` loader and keep `v3/talent-application.js` as the single canonical Xano writer.
