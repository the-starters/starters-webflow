...
- `v3/auth-page-loader.js` — minimal V3 login and `/auth-route` runtime loader; on those three paths only it inserts `auth-route.js` and lets the site head skip the controllers unrelated to authentication and attribution, and it completes the timestamp-only timing receipt on the final destination. The sitewide parser-blocking `route-guard.js` and `signup-attribution.js` tags stay static and unconditional on every page — `route-guard.js` is the sole owner of guard delivery and guard-before-router ordering, and the loader never inserts a copy of it — and the loader admits the full controller block whenever it cannot install its own runtime; see [`v3/AUTH-ROUTE-WIRING.md`](v3/AUTH-ROUTE-WIRING.md)
