...
- `v3/brand-account-controller.js` — Memberstack-first Brand Build Account controller for the native `/complete-profile` form, plus a role-scoped Account Security email controller. Its sitewide mode uses the canonical route-guard role contract to claim Brand forms only, leaves Talent/unmapped forms Memberstack-native, and sends one reset/set-password email rather than a separate verification message. Xano remains downstream of Memberstack webhook #1513, keyed by stable member ID, and `complete-profile-photo.js` binds the authored upload control to Memberstack's profile-image action. The authoritative contract and canaries live in [`v3/BRAND-ACCOUNT-WIRING.md`](v3/BRAND-ACCOUNT-WIRING.md)
