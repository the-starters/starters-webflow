...
- `global-embeds/expert-card/expert-card.css` — **CDN-served** company-list companion for the card; load in **Head** from the same jsDelivr ref as `expert-card.js`, not a paste embed. Takes `.expert-card_company-list` out of flex so companies read as one sentence, clamps it to `max-height: 2lh`, and replaces the leading `::before` comma with an `::after` comma glued to its own company (the gap to the next one is margin, so a wrap can never open on a comma), with last-child suppression for all three markup shapes (CMS nested collection, Algolia template siblings, plain paragraphs) ([docs](https://wf-starter-embeds-docs.vercel.app/docs/global-embeds/expert-card))
