...
- `quiz-main/quiz-attribution.js` — sitewide UTM/Meta ad-attribution capture; copies `utm_source`/`utm_campaign`/`utm_adset`/`utm_content`/`fbclid` from the landing URL into same-named first-party cookies (72h, `path=/`), falls back to Meta's `_fbc`/`_fbp` cookies for `fbc`/`fbp`, mints a reusable `event_id` (`evt_<uuid>`) for CAPI dedup, and on both `/quiz` and `/sign-up` fires Meta `CompleteRegistration` (with `eventID`) on the logged-out -> logged-in signup transition, once per session; on `/sign-up` also persists the eight attribution cookies to Memberstack itself (marker + field snapshot, retried sitewide after the form's `/brand-dashboard` redirect), while `/quiz` funnel persistence stays with `quiz-results.js`; requires the Meta Pixel base snippet in site-head custom code (see `quiz-main/README.md`)
...
- `v3/starter-dashboard-messages.js` — `/starter-dashboard` Messages tile: binds each `data-messages-element="wrapper"` instance to the member's recent TalkJS conversations by merging Xano `starter/messages/recent` (TalkJS REST proxy, so already-read conversations appear) with the TalkJS JS SDK `session.unreads` (live unread state, sender name/photo enrichment, unread-count badge); all instances share one TalkJS session and one Xano fetch, and the tile degrades to unreads-only when the Xano endpoint is unavailable. See [`v3/README.md`](v3/README.md#starter-dashboard-messages-tile)
...
- `v3/complete-profile-loader.js` — `/complete-profile` **submit spinner and backdrop dim**: shows the authored `[data-complete-profile-loader]` element while the Complete-profile form is submitting, and fades back `[data-complete-profile-element="form"]` and `[data-complete-profile-element="profile-photo"]` behind it. Pure observer of `aria-busy` on the form, which `v3/brand-account-controller.js`'s `setBusy()` already maintains — it binds no `submit` handler and never touches the submit button, because double-submit is already the controller's guard and two owners of one button is how a form ends up permanently disabled. Show/hide are **inline** `display: flex` / `display: none` writes, since the Designer's Display:None compiles to a class rule that a stylesheet write would lose to. Minimum display comes from the loader's own `data-loader` attribute (1000 as authored, 200 when missing or non-numeric) so a fast failure cannot flash the spinner, and every show arms a **5000ms fail-open cap** that hides the loader and restores the dim no matter what `aria-busy` says — a full-page overlay must never be able to trap a member. Rapid toggles coalesce into one session, with the dim bookkeeping outliving the hide by one 200ms transition so a re-show reuses the page's original inline values instead of recording its own dim as authored. Missing dim targets are skipped individually and silently (both attributes are still pending Designer work), a missing loader is an immediate zero-side-effect bail that makes the file safe to load site-wide, and the loader is force-hidden once at init as a self-heal against an authored-visible loader. No network, no Memberstack, no role contract. Wiring: [`v3/COMPLETE-PROFILE-LOADER-WIRING.md`](v3/COMPLETE-PROFILE-LOADER-WIRING.md)
