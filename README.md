...
- `v3/auth-page-loader.js` — minimal V3 login and `/auth-route` runtime loader; on those three paths only it inserts `route-guard.js` before `auth-route.js` and omits unrelated application controllers, leaving the sitewide parser-blocking `route-guard.js` tag untouched everywhere else, and completes the timestamp-only timing receipt on the final destination; see [`v3/AUTH-ROUTE-WIRING.md`](v3/AUTH-ROUTE-WIRING.md)
