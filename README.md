...
- `v3/starter-dashboard-stripe-connect.js` — Memberstack-scoped V3 Stripe Connect dashboard and OAuth-callback controller; paints loading/disconnected/incomplete/ready/review/error states from the Xano-native Stripe mirror, starts OAuth or onboarding through Xano, rejects cross-member callback state, and re-reads canonical status after the return instead of waiting on the V2 nightly CMS batch
