...
- `v3/starters-ms-redirect.js` — per-page Memberstack signup redirect: copies the target out of a hidden, CMS-bindable `starters-ms-redirect` marker onto every `form[data-ms-form="signup"]` (`redirect` + `data-redirect`) before Memberstack's `initSignupForms()` reads it, so a signup modal in a shared component or on a CMS template can send members back to the page they signed up from — something Webflow's one-static-value-per-form Redirect URL cannot do. Accepts only root-relative same-origin paths (rejects `//`, `/\`, and control characters); an author's non-empty Designer `redirect` always wins. See [`v3/README.md`](v3/README.md#signup-redirect-marker)
- `v3/signup-attribution.js` — sitewide UTM/Meta ad-attribution capture; copies `utm_source`/`utm_campaign`/`utm_adset`/`utm_content`/`fbclid` from the landing URL into same-named first-party cookies (72h, `path=/`), falls back to Meta's `_fbc`/`_fbp` cookies for `fbc`/`fbp`, mints a reusable `event_id` (`evt_<uuid>`) for CAPI dedup, and at the signup transition derives two more cookies from the page itself: `signup_source` (normalized path of the page the form was on) and `signup_referrer` (normalized path of the same-origin page it was reached from, silent for an absent or cross-origin referrer), neither of which any URL parameter can supply. It fires Meta `CompleteRegistration` (with `eventID`) on the logged-out -> logged-in signup transition, once per session, on every page it arms the signup watch for: the hand-audited `/quiz` and `/sign-up` paths, plus any page carrying a `form[data-ms-form="signup"]` and no `[data-ms-form="login"]` marker — form detection, so the `/all-starters` signup modal and any future signup surface are covered with no Designer work and no edit here. It also persists the ten attribution cookies to Memberstack itself on every armed page except `/quiz` (marker + field snapshot, retried sitewide after the signup form's own redirect cuts the write off), while `/quiz` funnel persistence stays with `quiz-results.js` so the two never race; the `signup_source` and `signup_referrer` cookies are the exception and are written on every armed page including `/quiz`, since that funnel's field write happens a page later and the cookie is its only carrier. Both are write-once on the member: once either holds a value no write replaces it, because a returning member logging in on a signup page is indistinguishable from a new signup, while the eight click fields stay last-touch so a fresh ad click still updates them. Requires the Meta Pixel base snippet in site-head custom code. See [`v3/README.md`](v3/README.md#signup-attribution)
...
- `v3/complete-profile-redirect.js` — `/complete-profile` **outbound** half of the Brand profile-completion loop (and wrong-role bounce): puts every **mapped** member who lands on the page where they belong, with **no hop through `/login`**. Free Brand → quiz funnel home from the sitewide `v3/route-guard.js` contract; Talent → `/starter-dashboard`. Paid Brand completion is read from Xano `starters_onboarding/get_brand_profile_status` (`{has_record, brand_profile_done}`) via trade-token — **same signal** as `v3/brand-profile-redirect.js` and the paid-Brand branch of `v3/auth-route.js` (changed 2026-08-06; the Memberstack field `completed-brand-profile` is still written by the controller for endpoint #1513 but nothing routes on it). A non-empty `thestarters:v3-brand-profile-completed` sessionStorage marker (written by `v3/brand-account-controller.js` after durable submit) short-circuits as done with no network call. Only `brand_profile_done === true` (or the marker) replaces to `/brand-dashboard`; unfinished or inconclusive answers stay. Fail-open means stay. Not the page access gate — Memberstack `restrict-pages` owns that. Wiring: [`v3/COMPLETE-PROFILE-REDIRECT-WIRING.md`](v3/COMPLETE-PROFILE-REDIRECT-WIRING.md) and [`v3/BRAND-PROFILE-REDIRECT-WIRING.md`](v3/BRAND-PROFILE-REDIRECT-WIRING.md)
- `v3/brand-profile-redirect.js` — **inbound** half of the Brand profile-completion loop on paid-Brand lock pages (`/brand-dashboard`, `/opportunities` + detail slug, `/all-starters`, `/messages`, plus `/starter-dashboard` and `/dashboard` for future-proofing): raises `[data-page-spinner]`, trade-token + `get_brand_profile_status`, `location.replace('/complete-profile')` only when `has_record === true && brand_profile_done === false`. Same Xano signal as outbound + auth-route; marker short-circuits. Fail-open elsewhere. No role table — Talent/free-Brand get `has_record: false` and stay. One deferred tag per page (or sitewide). Wiring: [`v3/BRAND-PROFILE-REDIRECT-WIRING.md`](v3/BRAND-PROFILE-REDIRECT-WIRING.md)
- `v3/complete-profile-back.js` — `/complete-profile` **"Go back to &lt;page&gt;"** escape hatch: turns the authored, hidden `[data-complete-profile-back]` wrapper into a working control. Needed because `v3/brand-profile-redirect.js` arrives via `location.replace()`, which destroys the history entry the browser's own back button would have used. Captures `document.referrer` on init when its origin passes the same host allowlist, mirrors it into `sessionStorage` under `thestarters:v3-complete-profile-back` (its own key — never the shared `thestarters:v3-brand-profile-completed` marker), and falls back to that stored value when the referrer is empty, which is what keeps the button alive across a reload. Stays hidden for an off-site or absent referrer and for the funnel/login pages plus **every page `v3/brand-profile-redirect.js` guards** — going "back" to one of those just bounces an unfinished Brand here again, so the two path lists must move together. Labels the `[data-complete-profile-back-label]` text from a curated map (Home, Learn, Sessions, Article, Playbook, Webinar, Events, Case Studies, Why Us, Functions, Industries; `/hire/<slug>` → the Starter's first name; anything unmapped → a bare `Go back`), reveals by removing the `hide` class and clearing an inline `display:none`, and binds one latched click on both the wrapper and the inner `button.clickable_btn` → `location.assign(<full referrer URL>)`. No network, no Memberstack, no role contract; a missing hook leaves the page exactly as authored. Wiring: [`v3/COMPLETE-PROFILE-BACK-WIRING.md`](v3/COMPLETE-PROFILE-BACK-WIRING.md)
...
- `v3/brand-account-controller.js` — Memberstack-first Brand signup and Build Account controller plus configuration-gated login-email ownership for Brand Account Security and the visible Talent edit-profile form. The authoritative signup mapping, identity scope, propagation contract, failure semantics, and reversible canaries live in [`v3/BRAND-ACCOUNT-WIRING.md`](v3/BRAND-ACCOUNT-WIRING.md)
...
- `global-embeds/session-video/session-video.js` — the three-minute preview gate on the Learn Sessions template (`/learn/sessions/<slug>`). One Vimeo video is served to everyone and playback is **clamped** for non-members at `data-session-video-cut` seconds (default 180), so the lock lives in the player rather than in a modal: dismissing the wall returns a frozen frame, and any play attempt reopens it. That is what makes a dismissible wall cost nothing — a blocking modal would still have a playing video underneath it. **Member state is resolved BEFORE mount**, unlike the clamp-later design first planned, because `fullscreen` is not a Vimeo embed option: it is governed by the iframe's `allowfullscreen` / `allow` attributes, and permissions policy is evaluated at iframe load and **never re-evaluated**, so removing the attribute afterwards does nothing. `window.memberReady` therefore gets a 1200ms budget; expiry mounts **gated** (fail closed, the opposite of `learn-cta-gate.js`, because a clamped member reloads and recovers while a leaked video does not — and this file never locks scroll), and a late resolution remounts at the same position. Bypass routes are closed at construction, not by CSS: `controls`, `keyboard` and `pip` off plus no `allowfullscreen` while gated, because arrow keys seek and picture-in-picture ships its own scrubber. **No modal id appears in the file**: `[data-session-video="signup-trigger"]` carries `modal.js`'s own `data-modal-trigger`, authored in the Designer, and this file only clicks it, so `modal.js` needs no new public API; a test pins that. Markup contract is `[data-session-video="root" | "stage" | "play" | "progress" | "signup-trigger"]` with `data-session-video-id` bound to the CMS `id-video-for-waching` field. Emits `session-video-preview-start` (gated viewers only), `-wall` and `-complete` as window events; no destination is wired (a `-resume` event arrives with ticket 06). `window.StartersSessionVideo` exposes `status()` and `reveal()`. Diagnostics on staging hosts and `STARTERS_DEBUG` only. Spec and tickets in `.scratch/sessions-video-gate/`
