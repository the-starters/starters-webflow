...
- `quiz-main/quiz-attribution.js` — sitewide UTM/Meta ad-attribution capture; copies `utm_source`/`utm_campaign`/`utm_adset`/`utm_content`/`fbclid` from the landing URL into same-named first-party cookies (72h, `path=/`), falls back to Meta's `_fbc`/`_fbp` cookies for `fbc`/`fbp`, mints a reusable `event_id` (`evt_<uuid>`) for CAPI dedup, and on `/quiz` fires Meta `CompleteRegistration` (with `eventID`) on the logged-out→logged-in signup transition, once per session; requires the Meta Pixel base snippet in site-head custom code and pairs with the `quiz-results.js` Memberstack field write (see `quiz-main/README.md`)
