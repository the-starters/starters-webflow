...
- `v3/profile-portfolio.js` — sole `/hire/<slug>` portfolio / case-study renderer for the section labelled "Highlights"; reads approved public Xano Portfolios (#28) via `Get_approved_portfolios`, binds by custom attribute with legacy class fallbacks, and carries a `data-portfolio-rendered` guard against repeated CDN execution. It renders the card list and **fills the Highlights modal with data only** — the lumos modal system (`global-embeds/modal/modal.js`) owns opening, closing, animation, focus and scroll locking, and this script never writes to the dialog. All modal lookups are scoped inside the modal root. The owner-read legacy on-canvas embed must be removed in the coordinated cutover. Wiring and cutover: [`v3/PROFILE-PORTFOLIO-WIRING.md`](v3/PROFILE-PORTFOLIO-WIRING.md)
