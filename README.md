...
- `quiz-main/quiz-attribution.js` — sitewide UTM/Meta ad-attribution capture: landing-URL params and Meta cookies into first-party cookies, a reusable CAPI `event_id`, the signup-transition `CompleteRegistration` event, and Memberstack persistence on `/sign-up`; the authoritative cookie contract, Memberstack field IDs, and Meta Pixel prerequisite live in [`quiz-main/README.md`](quiz-main/README.md#ad-attribution)
...
- `quiz-main/quiz-redirect.js` — `/quiz` member entry redirect by role and quiz state (with the ready-payload safety net for just-signed-up members); authoritative rules live in [`quiz-main/README.md`](quiz-main/README.md#entry-redirect)
...
- `v3/auth-route.js` — V3-only login/signup router on `/login`, `/starter-login`, and `/auth-route`; consumes the sitewide route guard's stable role contract, applies role-scoped `next` destinations, and forks Talent and paid-Brand logins on their Xano funnel-position reads, failing open; the authoritative routing table, failure semantics, and install map live in [`v3/README.md`](v3/README.md#login-router) and [`v3/AUTH-ROUTE-WIRING.md`](v3/AUTH-ROUTE-WIRING.md)
...
- `v3/route-guard.js` — sitewide V3 stable role resolver, canonical `/dashboard` router, and direct-access guard for protected role-scoped pages (plus the public-entry and funnel-page bounce rules); the authoritative page rules, redirect semantics, and install order live in [`v3/README.md`](v3/README.md#protected-route-guard) and [`v3/ROUTE-GUARD-WIRING.md`](v3/ROUTE-GUARD-WIRING.md)
- `v3/password-recovery.js` — V3-only shared Brand/Talent password recovery on the canonical `/forgot-password -> /reset-password -> /password-success` flow, preserving reset-token query strings across legacy-path redirects; see [`v3/README.md`](v3/README.md#shared-password-recovery)
...
- `v3/starters-ms-redirect.js` — per-page Memberstack signup redirect: copies the target from a hidden, CMS-bindable `starters-ms-redirect` marker onto every signup form before Memberstack reads it; the authoritative markup contract and path-safety rules live in [`v3/README.md`](v3/README.md#signup-redirect-marker)
...
- `v3/saved-starters-roles.js` — `/favorites` saved-list roles chips: one cloned paragraph per delimited role, so the wf-xano saved list matches the Algolia browse cards; authoritative behavior and constraints live in [`v3/README.md`](v3/README.md#saved-starters-roles-chips)
...
- `v3/starter-dashboard-messages.js` — `/starter-dashboard` Messages tile: the member's recent TalkJS conversations, merged from the Xano TalkJS REST proxy and live SDK unreads; the authoritative wiring and data contract live in [`v3/README.md`](v3/README.md#starter-dashboard-messages-tile)
- `v3/starter-dashboard-points.js` — authenticated, attribute-driven `/starter-dashboard` points and rank tile; Webflow owns the markup and authored state cards, the script binds only Xano-canonical values; see [`v3/README.md`](v3/README.md#starter-dashboard-points-and-rank-tile)
...
- `v3/onboarding-profile-preview.js` — onboarding completion page self-preview card: the `beforeRender` transform for the wf-xano list (computed roles/category/location/bio fields plus endpoint-based arming); the authoritative page wiring, markup, and instance rules live in [`v3/ONBOARDING-PROFILE-PREVIEW-WIRING.md`](v3/ONBOARDING-PROFILE-PREVIEW-WIRING.md) and [`v3/README.md`](v3/README.md#onboarding-profile-preview)
- `v3/onboarding-done-redirect.js` — read half of the `/starter-onboarding` completion pair: redirects an already-done member to `/starter-dashboard`, failing open everywhere because it is UX and never a security boundary; installs only as a pinned pair with `v3/patch-onboarding-status.js`; the authoritative wiring, QA order, and troubleshooting live in [`v3/ONBOARDING-DONE-REDIRECT-WIRING.md`](v3/ONBOARDING-DONE-REDIRECT-WIRING.md)
- `v3/patch-onboarding-status.js` — write half of the same pair: detects completion from Webflow's own form success state, records `onboarding_done` in Xano with retries, and routes the member to `/starter-dashboard`; installs only as a pinned pair with `v3/onboarding-done-redirect.js`; the authoritative wiring lives in [`v3/ONBOARDING-PATCH-STATUS-WIRING.md`](v3/ONBOARDING-PATCH-STATUS-WIRING.md)
- `v3/build-profile-redirect.js` — `/build-profile/*` funnel-position redirect for signed-in Talent arriving via bookmark, back button, or stale link; fail-open funnel UX, never a security boundary; the authoritative rules and page embeds live in [`v3/README.md`](v3/README.md#build-profile-funnel-redirect) and [`v3/BUILD-PROFILE-REDIRECT-WIRING.md`](v3/BUILD-PROFILE-REDIRECT-WIRING.md)
- `v3/complete-profile-redirect.js` — `/complete-profile` outbound half of the Brand profile-completion loop: wrong-role bounce plus the done-paid-Brand exit to `/brand-dashboard`, fail-open means stay; the authoritative routing and signals live in [`v3/README.md`](v3/README.md#complete-profile-role-routing) and [`v3/COMPLETE-PROFILE-REDIRECT-WIRING.md`](v3/COMPLETE-PROFILE-REDIRECT-WIRING.md)
- `v3/brand-profile-redirect.js` — inbound half of the same loop on the paid-Brand lock pages: sends an unfinished paid Brand to `/complete-profile`, everyone else stays, fail-open; the authoritative page list and Xano signal live in [`v3/BRAND-PROFILE-REDIRECT-WIRING.md`](v3/BRAND-PROFILE-REDIRECT-WIRING.md)
- `v3/complete-profile-back.js` — `/complete-profile` "Go back to `<page>`" escape hatch: referrer-based, no network, stays hidden for off-site referrers and guarded pages; the authoritative behavior, label map, and path lists live in [`v3/README.md`](v3/README.md#complete-profile-back-button) and [`v3/COMPLETE-PROFILE-BACK-WIRING.md`](v3/COMPLETE-PROFILE-BACK-WIRING.md)
...
- `account-settings/plan-dates.js` — Memberstack plan/billing dates as text via the `ms-form-pause-*` attributes (signup, next-billing, and pause resume dates, member-selectable pause length, reveal gating); read-only and fail-quiet; the authoritative attribute reference, anchor semantics, and reveal CSS live in [`account-settings/README.md`](account-settings/README.md#memberstack-plan-dates)
- `v3/xano-grabber/xano-grabber.js` — live DOM mirror: copies a wf-xano-rendered value into every element sharing its `wf-xano-grab-id` (single values and list mode), with real-content gating and never-revert; no network calls, no storage; the authoritative attribute table, gating rules, and pinned embed live in [`v3/xano-grabber/XANO-GRABBER-WIRING.md`](v3/xano-grabber/XANO-GRABBER-WIRING.md) and [`v3/README.md`](v3/README.md#xano-grabber-live-value-mirror)
...
- `v3/scheduling-availability-init.js` — scheduling availability control
  ...
  boundary
- `v3/scheduling-availability-writer.js` — writer counterpart to the availability initializer: availability form submit, manager selection, Nylas scheduler configuration, timezone set, and the calendar OAuth grant redirect; see the [`v3/README.md` booking-stage availability writer section](v3/README.md#booking-stage-availability-writer) for its authoritative host, path, and safety boundary
...
- `account-settings/ms-form-cancel-state.js` — picks one of several success messages on the `/account-settings` cancel flow from the reason button the member clicked; the authoritative attribute table and design rationale live in [`account-settings/README.md`](account-settings/README.md#cancel-flow-success-state)
