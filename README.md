...
- `global-embeds/form-embeds/password-validation/password-validation.js` — password-requirements checklist with whole-form CTA gating: the `[ms-code-submit-button]` CTA (including Memberstack's `.clickable_btn` overlay and every control under the marker) stays disabled until the active password rules pass plus the form's terms checkbox and a plausible email when present; an enabled non-submitting overlay click dispatches a cancelable synthetic submit so Memberstack receives it (never a native submission), on every Memberstack form (`data-ms-form`) carrying the marker whether or not a checklist is present (so the script must stay loaded site-wide wherever Memberstack forms use the Button component), and post-submit Memberstack/Turnstile rejections paint the form's `.w-form-fail` block as a `role="alert"`. Each instance picks its rule set through `starters-password-validation-*` wrapper attributes, misconfigured instances fail open, diagnostics staging-gated. On a Memberstack auth form (login, signup, forgot-password, reset-password) carrying the marker the CTA is gated even with no checklist present, on the fields that form actually has (a non-empty password, a plausible email, a checked terms box), while profile and security forms are bridged but never gated. Call `window.startersPasswordValidation.rescan()` for forms added after load, and `window.startersPasswordValidation.regate(form)` when a peer script that borrowed one form's CTA hands it back: it re-adjudicates that form's gate alone, so the page-level staging warnings do not re-print ([docs](https://wf-starter-embeds-docs.vercel.app/docs/global-embeds/form-embeds/password-validation))
- `global-embeds/form-embeds/memberstack-loader/memberstack-loader.js` — makes a Memberstack auth form's Button (login, signup, forgot-password, reset-password) look and behave busy and disabled while Memberstack shows its spinner, and restores it on hide; a successful submit keeps the button pending until Memberstack's redirect takes the page. The busy look and the double-submit guard are for auth forms only, as the file header states; loader routing and the one-request-at-a-time rule below cover every Memberstack form, auth and profile alike, and the submitting form's own Button Spinner is the one that turns. Two paths ship. A page that authors `data-ms-loader` keeps the attribute where it is, because Memberstack pins the element it cached at load, and the script mirrors that element's show and hide onto the submitting form's Spinner. A page that authors none gets the attribute placed on the submitting form's Spinner at submit time. A form with no Spinner leaves Memberstack to its full-screen overlay, whose hide lands on whichever Spinner holds the marker by then and so strands the overlay on screen; that overlay is not tracked and takes no part in the one-request rule below, and giving those forms the Button component is the fix. A double-submit guard means repeat submits while the button is pending never reach Memberstack. Because a Memberstack hide carries no identity and lands on whichever element holds the marker at that moment, only one Memberstack request may be open per page. The refusal is keyed to the request this script itself witnessed: while the element it lit or routed is still lit, every submit is refused, a second click on the form that opened the request included, and staging says so in the console; on a page that authors no loader, a Button Spinner taken off the page mid-request releases the page rather than locking it, so that request's hide may land on the next form's Spinner. A marker lit by anything else, such as a duplicate marker or Memberstack's own logout path, which shows the loader and never hides it, refuses no other form, though a form whose own Button Spinner is the pinned marker still reads that light as its own request. A page restored with the back button, which Memberstack will never hide the loader for, clears the state, puts the button back and takes any leftover full-screen overlay off the page. On hide the button is handed back to password-validation: with password-validation v1.59.510 or newer the loader calls `regate(form)`, which re-adjudicates that one form's gate; when `regate` is missing, or declines a form it never bridged, the loader falls back to `rescan()`, which re-gates the whole page and re-prints that script's page-level staging warnings; with password-validation older than v1.59.504 the hand-back still runs, but that release has no ownership marks and undoes its own gate on the way back, so pin password-validation at v1.59.504 or newer (v1.59.510 or newer for `regate`). Neither script can enforce the pin, and the value on the page cannot settle it either: releases before v1.59.504 expose no release marker at all, which staging names once in the console, and v1.59.504 and v1.59.505 report themselves as v1.59.502. The ownership marks `data-memberstack-loader-theme`, `data-memberstack-loader-busy` and `data-memberstack-loader-aria` are written at runtime and must never be authored in Webflow; other than the theme it always overwrites, only marked attributes are ever removed, so a peer script's hold on the same CTA survives. Call `window.startersMemberstackLoader.rescan()` for forms added after load, and read `window.startersMemberstackLoader.release` for the version in the page. Load it site-wide from the same global embed as password-validation, after that script: both bind capture-phase submit listeners on the same form at their own init, and same-phase listeners run in registration order, so loading password-validation first lets its gate adjudicate a submit before this script's refusal does. Pin both to a version tag. Designer contract: put `data-ms-loader` on the Button Spinner; a missing Spinner degrades to Memberstack's overlay (or to the pinned loader on a page that authored one), and the warning about that names the form and the reason and is one of the [staging-only console diagnostics](#staging-only-console-diagnostics). Source: [memberstack-loader.js](global-embeds/form-embeds/memberstack-loader/memberstack-loader.js)
