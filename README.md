...
- `navbar-embeds/memberstack/ms-code-field-link.js` — binds placeholder `[ms-code-field-link]` elements to member-specific destinations. Other fields keep their external-link behavior; `freelancer-profile-url` instead resolves the current member through Xano to the internal `/hire/<slug>` route and never reads the legacy V2 field value. The disabled link stays visible with its authored label while identity or resolver state is unresolved. A successful response with no published V3 slug changes the label to `View Profile (Publishing)`. In both disabled states, the link applies the class named by `data-class-disabled`, has no navigation attributes, and polls until it can repaint as the active profile link
