...
- `v3/hire-profile.js` — `/hire/<slug>` profile renderer, ported out of the page footer custom code so the logic lives in GitHub: public Experiences and Clients, member-gated booking wiring, Services call-card visibility derived from the public Algolia record for anonymous and brand viewers, cloned Freelance/Retainer rate cards, and the small page utilities that shipped in the same footer. Reads the Algolia index from the page rather than hardcoding it. Wiring, dependencies, and verification: [`v3/HIRE-PROFILE-WIRING.md`](v3/HIRE-PROFILE-WIRING.md)
