...
- `v3/agency-profile.js` — `/hire/<slug>` Agency section: agency name, team size, contract type, and the intro video, read from the public Xano endpoint `profile/starter/agency/v1`. `wf-xano` owns the fetch, the text binds, and every show/hide decision; this script only supplies the profile slug the URL path carries (which `wf-xano` cannot source), sets the video iframe's `src` (which `wf-xano` has no documented binder for), and collapses the wrapper on non-agency profiles so a hidden section stops consuming its parent's flex/grid gap. Non-agency profiles are unaffected and the section fails closed in every direction. The authoritative attribute contract, markup, install, and verification live in [`v3/AGENCY-PROFILE-WIRING.md`](v3/AGENCY-PROFILE-WIRING.md)
