...
- `account-settings/msform-cancel-state.js` — one success message out of several on the `/account-settings` cancel flow, picked by the reason button the member clicked. Inside a `[msform-cancel-state]` root, a click on `[msform-cancel-state-change="<key>"]` shows every `[msform-cancel-state-element="success-item"][msform-cancel-state-key="<key>"]` and hides the rest; the key is the entire contract (compared trimmed and case-sensitively, never by DOM order or position), `default` is the pre-click state and is reachable as a key like any other, and a root value naming an existing item starts there while any other value is treated as an instance label rather than a state, so `msform-cancel-state="cancel-form"` cannot strand the form on a state with no message. State is per root and every lookup is ownership-filtered to the root that is an element's **nearest** `[msform-cancel-state]` ancestor, so several cancel forms on one page — or a nested pair — cannot repaint each other; one delegated `document` click listener in the **capture** phase covers all of them, which also means buttons and items Webflow renders later (CMS, a component, a modal that mounts on open) need no re-init and a `stopPropagation()` below cannot swallow the state change. The click is deliberately never `preventDefault()`-ed, because these buttons double as the `global-embeds/step-flow/step-flow.js` Continue control (which does its own) and as the form's submit button. Both attributes go on Webflow's `.button_main-wrap` component wrapper, the same element the page's other systems already tag; the real click lands on the overlaid `.clickable_btn` or on `.button_main-text` and never on the wrapper, so the trigger is resolved with `closest()` and the inner `type` is irrelevant (branch buttons are `type="button"`, the final Confirm is `type="submit"`, both work). A control another system has gated is skipped — `data-button-theme="disabled"`, `data-form-flow-disabled`, or `aria-disabled="true"` anywhere between the trigger and the root, plus a native `disabled` — because step-flow disables that wrapper by attribute rather than by property, so a gated Continue still *receives* the click and is only `preventDefault()`-ed; capture phase means this script would otherwise see it first and repaint the success message for a step the flow refused to advance. Nothing is inferred from labels, `data-button-style`, or bare `<button>` elements the way step-flow does, so a tab control or lookalike footer button can never be hijacked. A `-change` value of `button`, `success-item`, or `success-wrapper` is reported as a swapped attribute pair (warned once per element, not once per click) and still falls back to the `default` item. Items are found inside the tagged `success-wrapper` when there is one and anywhere the root owns when there is not, and when the root turns up nothing the search falls back to the enclosing `.w-form` — that is what makes the root work tagged on the Form Block *or* on the `<form>`, where Webflow emits the success div as a sibling rather than a descendant. Painting happens at click time, not on submit: the success div is in the DOM from page load, just hidden, so there is nothing to listen for and no race with Webflow's AJAX submit, and a member who changes their mind repaints immediately. Every matching item shows rather than the first, since a key is a state and not an element id. The authored display is read **before** anything is hidden and restored on show, so flex and grid items keep their layout, with Designer-hidden items (computed `none`, which nothing can be restored to) falling back to `block` unless `msform-cancel-state-display` says otherwise. Reflects `msform-cancel-state-current` on the root and `msform-cancel-state-active` on the live button as CSS/QA hooks, always un-prefixed, plus `aria-hidden` per item; the `data-` spelling of every attribute is accepted on input so a mixed-spelling page cannot fail silently. A key with no item renders `default` instead of an empty success box while still reporting the requested key, and diagnostics are staging-only. Not persisted — in-page state survives Webflow's AJAX submit but not a redirect or reload; `window.StartersMsFormCancelState` exposes `get`/`set`/`refresh` for QA and late DOM
